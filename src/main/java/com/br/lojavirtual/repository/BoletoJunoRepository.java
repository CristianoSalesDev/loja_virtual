...
public interface BoletoJunoRepository extends JpaRepository<BoletoJuno, Long> {
	...
	@Query("select b from BoletoJuno b where b.pedidoId.id = ?1 and b.quitado = false")
	...
	@Query(nativeQuery = true, value = "update t_boleto_juno set quitado = true where code = ?1")
	public void quitarBoleto(String code);
	
	
	@Transactional
	@Modifying(flushAutomatically = true)
	@Query(nativeQuery = true, value = "update t_boleto_juno set quitado = true where id = ?1")
